...
export function transformNullableFormValue<T>(value: T, nullify = true): T | undefined | null {
...
export function transformNullableFormNumber<K extends boolean = true>(
  ...
  if (typeof value === 'undefined' || (typeof value === 'string' && value.trim() === '')) {
    return (nullify ? null : undefined) as K extends true ? number | null : number | undefined;
  }
...
export function transformNullableFormNumbers<
  ...
  K extends boolean = true
>(data: T, nullify: K = true as K): K extends true ? NullableNumbers : OptionalNumbers {
  return Object.entries(data).reduce(
    (acc, [key, value]) => {
      return {
        ...acc,
