        ...
        {dynamicResults.map(group => {
          return (
            <CommandGroup
              key={group.title}
              heading={group.title}
              data-testid={`search-group-${group.area}`}
            >
    ...
    <RadixDialog.Root {...props}>
      <RadixDialog.Portal>
        ...
        >
          ...
          <CommandPalette
            className="[&_[cmdk-group-heading]]:text-muted-foreground flex h-full flex-col overflow-hidden [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group]:not([hidden])_~[cmdk-group]]:pt-0"
            data-testid="search-command-palette"
          >
            {children}
          </CommandPalette>
          <div
            className="flex items-center justify-end border-t bg-ui-bg-field px-4 py-3 text-ui-fg-subtle"
            data-testid="search-modal-footer"
          >
            <div className="flex items-center gap-x-3">
              <div
                className="flex items-center gap-x-2"
                data-testid="search-navigation-hint"
              >
                <Text
                  size="xsmall"
                  leading="compact"
              ...
              <div
                className="flex items-center gap-x-2"
                data-testid="search-open-result-hint"
              >
                <Text
                  size="xsmall"
                  leading="compact"
...
>(({ className, value, onValueChange, area, setArea, isFetching, onBack, ...props }, ref) => {
  ...
  return (
    ...
    >
      <div
        className="px-4 pt-4"
        data-testid="search-area-selector"
      >
        <DropdownMenu
          dir={direction}
          data-testid="search-area-dropdown"
        >
              ...
              {SEARCH_AREAS.map(area => (
                <Fragment key={area}>
                  ...
                  <DropdownMenu.RadioItem
                    value={area}
                    data-testid={`search-area-option-${area}`}
                  >
                    {t(`app.search.groups.${area}`)}
                  </DropdownMenu.RadioItem>
      ...
      <div
        className="relative flex items-center gap-x-2 px-4 py-3"
      ...
      >
        ...
        />
        <div
          className="absolute end-4 top-1/2 flex -translate-y-1/2 items-center justify-end gap-x-2"
          data-testid="search-input-actions"
        >
          {isFetching && (
            <Spinner
              className="animate-spin text-ui-fg-muted"
...
>((props, ref) => {
  ...
  return (
    ...
    >
      <div
        className="flex min-h-[236px] flex-col items-center justify-center gap-y-3 text-ui-fg-subtle"
        data-testid="search-empty-content"
      >
        <MagnifyingGlass
          className="text-ui-fg-subtle"
          data-testid="search-empty-icon"
        />
        <div
          className="flex flex-col items-center justify-center gap-y-1"
          data-testid="search-empty-text"
        >
          <Text
            size="small"
            weight="plus"
...
>((props, ref) => {
  return (
    ...
    >
      <div
        className="w-full px-2 pb-1 pt-3"
        data-testid="search-loading-header"
      >
      ...
      {Array.from({ length: 7 }).map((_, index) => (
        <div
          ...
          data-testid={`search-loading-item-${index}`}
        >
