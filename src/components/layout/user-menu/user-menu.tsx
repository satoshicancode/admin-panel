  ...
  return (
    <div data-testid="sidebar-user-menu">
      <DropdownMenu
        ...
        data-testid="sidebar-user-menu-dropdown"
      >
        <UserBadge />
        <DropdownMenu.Content
          className="min-w-[var(--radix-dropdown-menu-trigger-width)] max-w-[var(--radix-dropdown-menu-trigger-width)]"
        ...
        >
          ...
          <DropdownMenu.Item
            asChild
            data-testid="sidebar-user-menu-profile-settings"
          >
            <Link
              to="/settings/profile"
          ...
          <DropdownMenu.Item
            asChild
            data-testid="sidebar-user-menu-documentation"
          >
            <Link
          ...
          <DropdownMenu.Item
            asChild
            data-testid="sidebar-user-menu-changelog"
          >
            <Link
              to="https://medusajs.com/changelog/"
              target="_blank"
            >
    ...
    >
      ...
      >
        <div
          ...
          data-testid="sidebar-user-menu-avatar-container"
        >
          {fallback ? (
            <Avatar
              size="xsmall"
        ...
        <div
          ...
          data-testid="sidebar-user-menu-name-container"
        >
          {displayName ? (
            <Text
              size="xsmall"
  ...
  return (
    <DropdownMenu.SubMenu data-testid="sidebar-user-menu-theme-toggle">
      <DropdownMenu.SubMenuTrigger
        ...
        data-testid="sidebar-user-menu-theme-trigger"
      >
      ...
      <DropdownMenu.SubMenuContent data-testid="sidebar-user-menu-theme-content">
        <DropdownMenu.RadioGroup
          value={theme}
          data-testid="sidebar-user-menu-theme-radio-group"
        >
  ...
  return (
    ...
    >
      <RadixDialog.Portal>
        ...
        />
        <RadixDialog.Content
          className="fixed left-[50%] top-[50%] flex h-full max-h-[612px] w-full max-w-[560px] translate-x-[-50%] translate-y-[-50%] flex-col divide-y overflow-hidden rounded-lg bg-ui-bg-subtle shadow-elevation-modal"
          data-testid="shortcuts-modal-content"
        >
          <div
            className="flex flex-col gap-y-3 px-6 py-4"
            data-testid="shortcuts-modal-header"
          >
            <div
              className="flex items-center justify-between"
              data-testid="shortcuts-modal-header-top"
            >
              <div data-testid="shortcuts-modal-title-container">
                <RadixDialog.Title asChild>
                  <Heading data-testid="shortcuts-modal-title">
                    {t('app.menus.user.shortcuts')}
                  </Heading>
                </RadixDialog.Title>
                <RadixDialog.Description
                  className="sr-only"
                  data-testid="shortcuts-modal-description"
                ></RadixDialog.Description>
              </div>
              <div
                className="flex items-center gap-x-2"
                data-testid="shortcuts-modal-close-container"
              >
                ...
                <RadixDialog.Close asChild>
                  <IconButton
                    variant="transparent"
                    size="small"
                    data-testid="shortcuts-modal-close-button"
                  >
                    <XMark />
                  </IconButton>
                </RadixDialog.Close>
          ...
          <div
            ...
            data-testid="shortcuts-modal-list"
          >
            {searchResults.map((shortcut, index) => {
              return (
                ...
                >
                  <Text
                    size="small"
                    data-testid={`shortcut-item-${shortcut.label.toLowerCase().replace(/\s/g, '-')}-label`}
                  >
                    {shortcut.label}
                  </Text>
                  <div
                    className="flex items-center gap-x-1"
                    data-testid={`shortcut-item-${shortcut.label.toLowerCase().replace(/\s/g, '-')}-keys`}
                  >
                    {shortcut.keys.Mac?.map((key, index) => {
                      return (
                        <div
                          className="flex items-center gap-x-1"
                          key={index}
                          data-testid={`shortcut-item-${shortcut.label.toLowerCase().replace(/\s/g, '-')}-key-${index}`}
                        >
                          <Kbd
                            data-testid={`shortcut-item-${shortcut.label.toLowerCase().replace(/\s/g, '-')}-kbd-${index}`}
                          >
                            {key}
                          </Kbd>
                          {index < (shortcut.keys.Mac?.length || 0) - 1 && (
                            <span
                              className="txt-compact-xsmall text-ui-fg-subtle"
                              data-testid={`shortcut-item-${shortcut.label.toLowerCase().replace(/\s/g, '-')}-separator-${index}`}
                            >
...
const UserItem = () => {
  ...
  return (
    <div
      className="flex items-center gap-x-3 overflow-hidden px-2 py-1"
      data-testid="sidebar-user-menu-item"
    >
      <Avatar
        size="small"
        variant="rounded"
      ...
      />
      <div
        className="block w-full min-w-0 max-w-[187px] overflow-hidden whitespace-nowrap"
        data-testid="sidebar-user-menu-item-details"
      >
        <Text
          size="small"
          weight="plus"
