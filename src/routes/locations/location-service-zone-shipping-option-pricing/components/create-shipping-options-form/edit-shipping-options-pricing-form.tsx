...
import { useRegions, useStore, useUpdateShippingOptions } from '@hooks/api';
...
export function EditShippingOptionsPricingForm({
  ...
  return (
    <RouteFocusModal.Form
      form={form}
      data-testid="location-shipping-option-pricing-form"
    >
      <KeyboundForm
          ...
          >
            ...
            >
              <div
                ...
                data-testid="location-shipping-option-pricing-form-container"
              >
                <DataGrid
                  isLoading={isLoading}
                  data={data}
          ...
          <div className="flex items-center justify-end gap-x-2">
            <RouteFocusModal.Close asChild>
              <Button
                variant="secondary"
                size="small"
                data-testid="location-shipping-option-pricing-form-cancel-button"
              >
                {t('actions.cancel')}
              </Button>
...
const getDefaultValues = (prices: HttpTypes.AdminShippingOptionPrice[]) => {
  ...
  prices.forEach(price => {
    ...
    if (hasAttributes(price, [REGION_ID_ATTRIBUTE], [ITEM_TOTAL_ATTRIBUTE])) {
      const regionId = price.price_rules.find(r => r.attribute === REGION_ID_ATTRIBUTE)?.value;
